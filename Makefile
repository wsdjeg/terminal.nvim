...
help:
	...
	@echo "  make test PATTERN=terminal              # Match test/**/*terminal*_spec.lua"
	@echo "  make test PATTERN=example               # Match test/**/*example*_spec.lua"
	@echo "  make test PATTERN=test/terminal_spec.lua  # Full path"
...
# Examples:
#   make test PATTERN=test/terminal_spec.lua
#   make test PATTERN=terminal  (shorthand for test/**/*terminal*_spec.lua)
test: install-deps
...
clean:
	...
	@rm -rf test/*.out
	@rm -rf *.swp
	@rm -rf /tmp/terminal_nvim_test_* 2>/dev/null || true
