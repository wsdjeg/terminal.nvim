...
local function get_test_files()
  ...
  if pattern:match('^test/') or pattern:match('^test\\') then
    ...
    -- e.g., "terminal" -> test/**/*terminal*_spec.lua
    files = vim.split(vim.fn.globpath('test', string.format('**/*%s*_spec.lua', pattern)), '\n')

    -- Also try exact match: e.g., "terminal_spec.lua"
    if #files == 0 then
      files = vim.split(vim.fn.globpath('test', string.format('%s*_spec.lua', pattern)), '\n')
    end
...
local function run_tests()
  ...
  print('=== terminal.nvim Test Suite ===')
...
local temp_pattern = '/tmp/terminal_nvim_test_'
